    ...
    const testMixpanel = () => {
        ...
        try {
            ...
            if (!token || token === 'YOUR_MIXPANEL_TOKEN_HERE') {
                setStatus('error');
                setMessage('❌ Token not configured. Add VITE_MIXPANEL_TOKEN to .env and restart server.');
                return;
            }
            ...
            setTimeout(() => {
                setStatus('success');
                setMessage('✅ Success! Go to Mixpanel → Live View to see "🧪 Mixpanel Test Event"');
            }, 500);

        } catch (error) {
            setStatus('error');
            setMessage(`❌ Error: ${error.message}. Check console.`);
