...
export default function GameMapElectronics() {
  ...
  const generateLevels = () => {
    return allLessons.map((lesson, index) => ({
      ...
      emoji: electronicsCurriculum.sections[lesson.sectionIndex]?.icon || '⚡',
      color: electronicsCurriculum.sections[lesson.sectionIndex]?.color || 'from-purple-400 to-pink-500',
      xp: lesson.xp || 150,
  ...
  const isLevelUnlocked = (levelId) => {
    ...
    return isLessonCompleted('electronics', levelId - 1);
  };
  const isLevelCompleted = (levelId) => isLessonCompleted('electronics', levelId);
  
  const progress = getSubjectProgress('electronics', totalLessons);
  ...
  const sections = electronicsCurriculum.sections.map((section, sectionIndex) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 text-white pb-20 md:pb-0">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        {[...Array(20)].map((_, i) => (
          ...
          >
            <Cpu className="w-full h-full text-purple-400" />
          </div>
        ))}
      </div>
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-bold">Electronics & Robotics Journey</h1>
                <p className="text-sm text-purple-200">{totalLessons} Lessons • Master electronics and robotics</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
                          ...
                          <div
                            className={`relative w-16 h-16 rounded-full border-4 flex items-center justify-center transition-all ${
                              completed
                                ...
                                : unlocked
                                ? `bg-gradient-to-br ${level.color} border-white shadow-lg shadow-purple-500/50 hover:scale-110`
          ...
          <div className="flex justify-center mt-12">
            <div className="bg-gradient-to-r from-yellow-400 to-amber-500 rounded-3xl p-8 border-4 border-yellow-300 shadow-2xl shadow-yellow-500/50">
              <div className="text-center">
                ...
                <div className="text-sm text-white/90 mt-1">Master Electronics Engineer</div>
              </div>
            </div>
          </div>
        </div>
